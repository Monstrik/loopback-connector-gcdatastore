...
describe('mongodb default id type', function () {
  ...
  it('should generate id value for create', function (done) {
    Account.create({
      ...
      Account.findById(account.id, function (err, account1) {

        if (err) return done(err);
        var nid = account1.__data.id;
        console.log('account1', JSON.stringify(account1.id))
        nid.should.be.eql(account.id);
        //account1[id].should.eql(account.id);
        account1.should.have.property('id');
        done(err, account1);
      });
    });
  });
